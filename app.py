...
def _make_channel_labeler(
    ...
    if ch_df is None or ch_df.empty:
        return lambda x: x
    m = dict(
        zip(ch_df["channel_id"].astype(str), ch_df["title"].fillna("").astype(str))
    )
...
def _ctx_ready() -> bool:
    return bool(st.session_state.get("api_key")) and bool(
        st.session_state.get("project_slug")
    )
...
def page_setup():
    ...
    with st.sidebar:
        ...
        outputs_root = st.text_input(
            "出力フォルダ",
            value=st.session_state.get("outputs_root", str(CFG.outputs_root)),
        )
        ...
        if st.button("適用", use_container_width=True):
            ...
            st.session_state["outputs_root"] = outputs_root.strip() or str(
                CFG.outputs_root
            )
...
def page_collect_by_search():
    st.header("データ収集")
    st.info(
        "日時は、YouTube Data APIの値に従い基本的にUTC（末尾Z）です。日本時間（JST）にするには +9時間してください。"
    )
    ...
    if use_search:
        st.subheader("2) 検索条件")
        q = st.text_input(
            "検索ワード（任意）", value=st.session_state.get("last_collect_query", "")
        )
    ...
    if use_channels:
        ...
        if parsed_channels.custom_usernames:
            st.warning(
                "/c/ や /user/ は Data API v3 だけで確実にチャンネルIDへ変換できません（必要ならチャンネルIDを貼ってください）。"
            )
    ...
    if use_search:
        ...
        est_rows.append(
            {"入口": "検索", "API": "search.list", "推定units": int(pages * 100)}
        )
        est_rows.append(
            {
                "入口": "検索",
                "API": "videos.list",
                "推定units": int((int(filters.get("total_results", 0)) + 49) // 50),
            }
        )
    if use_channels:
        ...
        est_rows.append(
            {
                ...
                "推定units": int(n_ch * pages),
            }
        )
        est_rows.append(
            {"入口": "チャンネル", "API": "videos.list", "推定units": int(n_ch * pages)}
        )
    if est_rows:
    ...
    if st.button(
        "収集を実行", type="primary", use_container_width=True, disabled=disabled
    ):
        conn = _open_conn()
        try:
            ...
            job_id = storage.create_job(
                ...
                "collect_conditions",
                {
                    ...
                    "include_broadcast_kinds": (
                        list(include_kinds) if include_kinds else None
                    ),
                },
            )
            conn.commit()
            ...
            if fetch_comments:
                ...
                # 重複除去・空除去
                vids_for_comments = list(
                    dict.fromkeys([v for v in vids_for_comments if v])
                )
                ...
                if vids_for_comments:
                    ...
                    st.warning(
                        "今回の収集でコメント対象の動画が無かったため、コメント収集はスキップしました。"
                    )
            ...
            if use_search:
                try:
                    ...
                    if res_search is not None:
                        storage.insert_search_run_videos(
                            conn, run_id, res_search.collected_video_ids
                        )
                except Exception:
                    pass
                conn.commit()
            ...
            if res_search is not None:
                msgs.append(
                    f"検索: 発見={res_search.video_ids_found}, 新規={res_search.new_videos}, 動画保存={res_search.videos_upserted}"
                )
            if res_channel is not None:
                msgs.append(
                    f"チャンネル: 発見={res_channel.video_ids_found}, 新規={res_channel.new_videos}, 動画保存={res_channel.videos_upserted}"
                )
            if res_comments is not None:
                msgs.append(
                    f"コメント: スレッド保存={res_comments.comments_threads_upserted}, コメント保存={res_comments.comments_upserted}"
                )
            st.success(" / ".join(msgs) if msgs else "完了")
            st.info(
                f"API呼び出し回数={api.stats.calls}, 推定クォータ={api.stats.quota_units}"
            )
    ...
    try:
        ...
        df = pd.DataFrame(
            [
                ...
                for j in jobs
                if j.job_type in ("collect_search_videos", "collect_conditions")
            ]
        )
    ...
    try:
        ...
        if dataset == "channels":
            ...
            df = pd.read_sql_query(
                "select * from search_runs order by collected_at desc", conn
            )
        ...
        if "channel_title" in view_df.columns:
            cols = [
                c for c in view_df.columns if c not in ("channel_title", "channel_id")
            ]
...
def page_stats_charts():
    ...
    try:
        ...
        with m4:
            st.metric(
                "長さ中央値（分）",
                round(float((df["duration_seconds"].fillna(0) / 60.0).median()), 2),
            )
        with m5:
            shorts_ratio = (
                float(df["is_short"].mean()) if "is_short" in df.columns else 0.0
            )
...
def page_snapshots():
    ...
    try:
        ...
        latest_n = st.number_input(
            "各チャンネルの最新N本", min_value=1, max_value=500, value=50, step=1
        )
        ...
        if st.button(
            ...
            disabled=vid_df.empty,
        ):
            targets: List[str] = []
            for cid in channel_pick:
                subset = (
                    vid_df[vid_df["channel_id"] == cid]
            ...
            rows.append(
                {
                    "ID": j.id,
                    "種別": (
                        f"{type_disp}（{j.job_type}）"
                        if type_disp != j.job_type
                        else j.job_type
                    ),
                    "開始": j.started_at,
                    "終了": j.ended_at,
                    ...
                    "エラー": (
                        "\n".join(d.get("errors", []))
                        if isinstance(d.get("errors"), list)
                        else (d.get("errors") or "")
                    ),
                }
            )
...
def page_data_dictionary():
                ...
                [
                    {
                        "カラム": "channel_id",
                        "意味（日本語）": "チャンネルID",
                        "説明": "UCから始まるID。動画やスナップショットのキー。",
                        "注意": "表示上は title（チャンネル名）を優先。",
                    },
                    {
                        "カラム": "title",
                        "意味（日本語）": "チャンネル名",
                    ...
                    },
                    {
                        "カラム": "custom_url",
                        "意味（日本語）": "カスタムURL",
                        "説明": "snippet.customUrl",
                        "注意": "未設定の場合はNULL。",
                    },
                    {
                        "カラム": "uploads_playlist_id",
                        "意味（日本語）": "uploadsプレイリストID",
                        "説明": "contentDetails.relatedPlaylists.uploads",
                        "注意": "初回取得時に保存。",
                    },
                    {
                        "カラム": "view_count",
                        "意味（日本語）": "総再生数",
                        "説明": "statistics.viewCount",
                        "注意": "取得時点の値。",
                    },
                    {
                        "カラム": "subscriber_count",
                        "意味（日本語）": "登録者数",
                        "説明": "statistics.subscriberCount",
                        "注意": "非公開の場合NULL。",
                    },
                    {
                        "カラム": "video_count",
                        "意味（日本語）": "公開動画数",
                        "説明": "statistics.videoCount",
                        "注意": "取得時点の値。",
                    },
                    {
                        "カラム": "fetched_at",
        ...
        st.dataframe(
            _df(
                [
                    ...
                    {
                        "カラム": "duration_seconds",
                        "意味（日本語）": "動画長（秒）",
                        "説明": "contentDetails.duration（ISO8601）を秒に変換。",
                        "注意": "Shorts判定などに使用。",
                    },
                    {
                        "カラム": "category_id",
                        "意味（日本語）": "カテゴリID",
                        "説明": "snippet.categoryId",
                        "注意": "カテゴリ名への変換は未実装。",
                    },
                    {
                        "カラム": "tags_json",
                        "意味（日本語）": "タグ（JSON）",
                        "説明": "snippet.tags の配列をJSON文字列で保存。",
                        "注意": "未設定の場合NULL。",
                    },
                    {
                        "カラム": "default_language",
                        "意味（日本語）": "既定言語",
                        "説明": "snippet.defaultLanguage",
                        "注意": "",
                    },
                    {
                        "カラム": "live_broadcast_content",
                        "意味（日本語）": "配信状態（API）",
                        "説明": "snippet.liveBroadcastContent（none/live/upcoming）",
                        "注意": "ライブアーカイブ判定にはこれだけでは不十分です。",
                    },
                    {
                        "カラム": "scheduled_start_time",
                        "意味（日本語）": "配信予定開始（UTC）",
                        "説明": "liveStreamingDetails.scheduledStartTime",
                        "注意": "予約動画/配信予定のときのみ。",
                    },
                    {
                        "カラム": "actual_start_time",
                        "意味（日本語）": "実開始（UTC）",
                        "説明": "liveStreamingDetails.actualStartTime",
                        "注意": "ライブ配信（配信中/アーカイブ）由来のときのみ。",
                    },
                    {
                        "カラム": "actual_end_time",
                        "意味（日本語）": "実終了（UTC）",
                        "説明": "liveStreamingDetails.actualEndTime",
                        "注意": "ライブアーカイブで入ることがあります。",
                    },
                    {
                        "カラム": "view_count",
                        "意味（日本語）": "再生数",
                    ...
                    },
                    {
                        "カラム": "like_count",
                        "意味（日本語）": "高評価数",
                        "説明": "statistics.likeCount",
                        "注意": "動画によりNULLのことがあります。",
                    },
                    {
                        "カラム": "comment_count",
                        "意味（日本語）": "コメント数",
                        "説明": "statistics.commentCount",
                        "注意": "コメント無効化等でNULL/0のことがあります。",
                    },
                    {
                        "カラム": "fetched_at",
                        "意味（日本語）": "取得日時",
        ...
        st.dataframe(
            _df(
                [
                    ...
                    {
                        "カラム": "author_channel_id",
                        "意味（日本語）": "投稿者チャンネルID",
                        "説明": "コメント投稿者（存在する場合）。",
                        "注意": "非公開等でNULLのことがあります。",
                    },
                    {
                        "カラム": "author_display_name",
                        "意味（日本語）": "投稿者表示名",
                        "説明": "コメント投稿者名。",
                        "注意": "",
                    },
                    {
                        "カラム": "text",
                        "意味（日本語）": "本文",
                        "説明": "textDisplay or textOriginal",
                        "注意": "絵文字やHTMLが含まれることがあります。",
                    },
                    {
                        "カラム": "published_at",
                        "意味（日本語）": "投稿日時",
            ...
            _df(
                [
                    ...
                    },
                    {
                        ...
                        "意味（日本語）": "チャンネルID",
                        "説明": "対象チャンネル。",
                        "注意": "画面/CSVでは channel_title を併記。",
                    },
                    {
                        "カラム": "channel_title",
                        "意味（日本語）": "チャンネル名（結合列）",
                        "説明": "channels.title をJOINした表示用列。",
                        "注意": "DB本体のchannel_snapshotsには存在しません（表示/出力時の結合）。",
                    },
                    {
                        "カラム": "view_count",
                        "意味（日本語）": "総再生数",
            ...
            _df(
                [
                    ...
                    },
                    {
                        ...
                        "意味（日本語）": "動画ID",
                        "説明": "対象動画。",
                        "注意": "画面/CSVでは video_title を併記。",
                    },
                    {
                        "カラム": "video_title",
                        "意味（日本語）": "動画タイトル（結合列）",
                        "説明": "videos.title をJOINした表示用列。",
                        "注意": "DB本体のvideo_snapshotsには存在しません（表示/出力時の結合）。",
                    },
                    {
                        "カラム": "channel_id",
                        "意味（日本語）": "チャンネルID",
            ...
            _df(
                [
                    ...
                    },
                    {
                        "カラム": "q",
                        "意味（日本語）": "検索語",
                        "説明": "search.list に渡したクエリ。",
                        "注意": "空文字のrunもあり得ます（チャンネル指定のみ収集など）。",
                    },
                    {
                        "カラム": "order_by",
                        "意味（日本語）": "並び順",
                        "説明": "date / relevance など。",
                        "注意": "",
                    },
                    {
                        "カラム": "mode",
    ...
    with tabs[6]:
        ...
        st.write(
            "Explore画面で選べる結合済みデータセットです（実体テーブルではなくJOIN結果）。"
        )
        st.dataframe(
            _df(
                [
                    ...
                    },
                    {
                        "カラム": "published_at",
                        "意味（日本語）": "公開日時",
                        "説明": "videos.published_at",
                        "注意": "UTCの可能性が高いです。",
                    },
                    {
                        "カラム": "view_count",
                        "意味（日本語）": "再生数",
                        "説明": "videos.view_count",
                        "注意": "取得時点の値。",
                    },
                    {
                        "カラム": "like_count",
                        "意味（日本語）": "高評価数",
                        "説明": "videos.like_count",
                        "注意": "",
                    },
                    {
                        "カラム": "comment_count",
                        "意味（日本語）": "コメント数",
                        "説明": "videos.comment_count",
                        "注意": "",
                    },
                    {
                        "カラム": "channel_title",
                        "意味（日本語）": "チャンネル名",
                        "説明": "channels.title（JOIN）",
                        "注意": "",
                    },
                ]
            ),
                ...
                [
                    {
                        "カラム": "channel_title",
                        "意味（日本語）": "チャンネル名（結合）",
                        "説明": "videos/comments/snapshots表示時にJOINで付与。",
                        "注意": "DBテーブルの列ではない場合があります。",
                    },
                    {
                        "カラム": "video_title",
                        "意味（日本語）": "動画タイトル（結合）",
                        "説明": "comments/snapshots表示時にJOINで付与。",
                        "注意": "",
                    },
                    {
                        "カラム": "published_date",
                        "意味（日本語）": "公開日（date型）",
                        "説明": "published_atから日付だけを抽出。",
                        "注意": "UTC基準で日付になることがあります。",
                    },
                    {
                        "カラム": "duration_min",
                        "意味（日本語）": "動画長（分）",
                        "説明": "duration_seconds/60",
                        "注意": "",
                    },
                    {
                        "カラム": "is_short",
                        "意味（日本語）": "Shorts判定",
                        "説明": "duration_seconds <= 60 をShortsとして扱う簡易判定。",
                        "注意": "厳密なShorts判定ではありません。",
                    },
                    {
                        "カラム": "broadcast_kind",
                        "意味（日本語）": "配信状態（推定）",
                        "説明": "live_broadcast_content と actual_start/end から推定（通常/ライブ配信中/予約/ライブアーカイブ）。",
                        "注意": "旧データは判定不可のことがあります。",
                    },
                    {
                        "カラム": "engagement_rate",
                        "意味（日本語）": "エンゲージメント率",
                        "説明": "(like_count + comment_count) / view_count",
                        "注意": "view_countが0/NULLの場合は0扱いにしています。",
                    },
                ]
            ),
            use_container_width=True,
