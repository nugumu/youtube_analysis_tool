...
def upsert_channels(conn: sqlite3.Connection, items: List[Dict[str, Any]]) -> int:
    now = utc_now_iso()
    rows: List[Tuple] = []
    for it in items:
        cid = it.get("id")
    ...
    conn.executemany(
        ...
        INSERT INTO channels(
          channel_id, title, description, published_at, country, custom_url, uploads_playlist_id,
          view_count, subscriber_count, video_count, fetched_at
        ) VALUES(?,?,?,?,?,?,?,?,?,?,?)
...
def upsert_videos(conn: sqlite3.Connection, items: List[Dict[str, Any]]) -> int:
    now = utc_now_iso()
    rows: List[Tuple] = []
    for it in items:
        vid = it.get("id")
    ...
    conn.executemany(
        """
        INSERT INTO videos(
          video_id, channel_id, title, description, published_at, duration_seconds, category_id,
          tags_json, default_language, live_broadcast_content, scheduled_start_time, actual_start_time, actual_end_time,
...
def insert_channel_snapshots(conn: sqlite3.Connection, channel_ids: List[str]) -> int:
    """Capture current stats from channels table into channel_snapshots."""
    now = utc_now_iso()
    q = "SELECT channel_id, view_count, subscriber_count, video_count FROM channels WHERE channel_id IN ({})".format(
    ...
    conn.executemany(
        "INSERT INTO channel_snapshots(channel_id, captured_at, view_count, subscriber_count, video_count) VALUES(?,?,?,?,?)",
        out_rows,
    )
    return len(out_rows)


def insert_video_snapshots(conn: sqlite3.Connection, video_ids: List[str]) -> int:
    now = utc_now_iso()
    q = "SELECT video_id, view_count, like_count, comment_count FROM videos WHERE video_id IN ({})".format(
        ",".join(["?"] * len(video_ids))
    )
    rows = conn.execute(q, video_ids).fetchall()
    out_rows = [(r["video_id"], now, r["view_count"], r["like_count"], r["comment_count"]) for r in rows]
    conn.executemany(
        "INSERT INTO video_snapshots(video_id, captured_at, view_count, like_count, comment_count) VALUES(?,?,?,?,?)",
        out_rows,
    )
    return len(out_rows)


def upsert_comment_threads_and_comments(conn: sqlite3.Connection, video_id: str, threads: List[Dict[str, Any]]) -> Tuple[int, int]:
    now = utc_now_iso()
    ...
    conn.executemany(
        """
        INSERT INTO comment_threads(thread_id, video_id, top_comment_id, total_reply_count, fetched_at)
        VALUES(?,?,?,?,?)
    ...
    conn.executemany(
        """
        INSERT INTO comments(comment_id, video_id, author_channel_id, author_display_name, text,
                             published_at, updated_at, like_count, fetched_at)
        VALUES(?,?,?,?,?,?,?,?,?)
...
def get_known_video_ids(conn: sqlite3.Connection, channel_id: Optional[str] = None) -> set:
    if channel_id:
...
def get_channel_uploads_playlist_id(conn: sqlite3.Connection, channel_id: str) -> Optional[str]:
    r = conn.execute("SELECT uploads_playlist_id FROM channels WHERE channel_id=?", (channel_id,)).fetchone()
    if not r:
        return None
    return r[0]


def create_job(conn: sqlite3.Connection, job_type: str, details: Dict[str, Any]) -> int:
    now = utc_now_iso()
    cur = conn.execute(
        "INSERT INTO jobs(job_type, started_at, status, details_json) VALUES(?,?,?,?)",
        (job_type, now, "running", json.dumps(details, ensure_ascii=False)),
    )
    return int(cur.lastrowid)


def finish_job(conn: sqlite3.Connection, job_id: int, status: str, details: Dict[str, Any]) -> None:
    now = utc_now_iso()
    conn.execute(
...
def list_jobs(conn: sqlite3.Connection, limit: int = 50) -> List[JobRecord]:
    rows = conn.execute(
        "SELECT id, job_type, started_at, ended_at, status, details_json FROM jobs ORDER BY id DESC LIMIT ?",
        (limit,),
    ).fetchall()
    out: List[JobRecord] = []
    for r in rows:
...
def create_search_run(
    ...
    *,
    ...
    now = utc_now_iso()
    cur = conn.execute(
        "INSERT INTO search_runs(q, order_by, mode, filters_json, collected_at) VALUES(?,?,?,?,?)",
        (q, order_by, mode, json.dumps(filters, ensure_ascii=False), now),
    )
    return int(cur.lastrowid)


def insert_search_run_videos(conn: sqlite3.Connection, run_id: int, video_ids: List[str]) -> int:
    if not video_ids:
        return 0
    rows = [(run_id, vid) for vid in video_ids]
    conn.executemany(
        "INSERT OR IGNORE INTO search_run_videos(run_id, video_id) VALUES(?,?)",
        rows,
    )
    return len(rows)
